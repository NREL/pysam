...
static int SAM_assign_from_dict(void* data_ptr, PyObject* dict, const char* tech, const char* group){
    ...
    while (PyDict_Next(dict, &pos, &key, &value)){
        ...
        if (PyNumber_Check(value)){
            ...
            if (SAM_has_error_msg(error, "Either parameter does not exist or is not numeric type.")) goto fail;

            float val = (float)PyFloat_AsDouble(value);

            error = new_error();
            func(data_ptr, val, &error);
            if (SAM_has_error(error)) goto fail;
        }
        // string
        else if (PyUnicode_Check(value)){
            SAM_error error = new_error();
            SAM_set_string_t func = SAM_set_string_func(SAM_lib_handle, "GenericSystem", "PowerPlant", name, &error);
            if (SAM_has_error_msg(error, "Either parameter does not exist or is not string.")) goto fail;
        ...
        // sequences
            ...
            if (PySequence_Check(first)){
                ...
                if (SAM_has_error_msg(error, "Either parameter does not exist or is not matrix type.")) goto fail;
            ...
            // array
                ...
                if (SAM_has_error_msg(error, "Either parameter does not exist or is not array type.")) goto fail;
