...
rm -rf *
/opt/_internal/cpython-3.7.13/bin/cmake ${SSCDIR} -DCMAKE_BUILD_TYPE=Release -DSAM_SKIP_TOOLS=1 -DSAMAPI_EXPORT=1 -DSAM_SKIP_TESTS=1 ../ssc/ || exit
/opt/_internal/cpython-3.7.13/bin/cmake --build . --target shared -j 6 || exit
/opt/_internal/cpython-3.7.13/bin/cmake --build . --target ssc -j 1 || exit

mkdir -p /io/build_linux_sam
cd /io/build_linux_sam
rm -rf *
/opt/_internal/cpython-3.7.13/bin/cmake ${SAMNTDIR}/api -DCMAKE_BUILD_TYPE=Release -DSAMAPI_EXPORT=1 -DSAM_SKIP_AUTOGEN=1 ../sam/api || exit
make -j 6 || exit
